...
from .forms import imgform, jml_pkt_forms, satuform,duaform,tigaform,empatform, FormGuru
...
def editguru(request):
    usr  = UserProfileInfo.objects.filter(user=request.user.id).first()
    idx = usr.id
    juduls1 = dataguru.objects.get(no_id=idx)
    template = 'vcustomer/editguru.html'
    if request.POST:
        form = FormGuru(request.POST, instance=juduls1)
        if form.is_valid():
            form.save()
            messages.success(request, 'Data Berhasil di Perbarui')
            return redirect('/customer/editguru', no_id=idx)
    else:
        form = FormGuru(instance=juduls1)
        konteks = {
            'form':form,
            'juduls1':juduls1
        }
...
def form(request,no):
    ...
    if request.POST:
        dataguru.objects.create(
         ...
         tanggal_lahir=request.POST['usia'],
...
class Cari(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        resulth = dataguru.objects.filter(
            Q(nama__contains=lw) |
            Q(kategori__contains=lw) |
            Q(alamat__contains=lw)
            )
        print(resulth)
        context = {
            "resulth":resulth,
...
def Form(request):
        return render(request, "vcustomer/form1.html", {})

def Upload(request):
        for count, x in enumerate(request.FILES.getlist("document")):
            def process(f):
                with open('/home/adi/django-apps/Jasa/media/document/file_' + str(count), 'wb+') as destination:
